...
const login = createAsyncThunk(
  ...
  async ({ username, password }, thunkAPI) => {
    try {
      ...
      thunkAPI.dispatch(
        uiActions.openAlert({ type: 'success', text: data.message })
      )
...
const authSlice = createSlice({
  ...
  extraReducers: {
    ...
    [loginWithToken.pending](state, { payload }) {
      state.isLoggingInWithToken = true
    },
    [loginWithToken.fulfilled](state, { payload }) {
      state.currentUserId = payload.data.user._id
      state.isLoggingInWithToken = false
    },
...
export const selectIsLoggingInWithToken = (state) => {
  return state.auth.isLoggingInWithToken
}
